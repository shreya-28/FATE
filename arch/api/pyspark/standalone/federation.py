...
from arch.api.pyspark.standalone.eggroll import _DTable
from arch.api.pyspark.standalone.eggroll import Standalone
...
def _get_meta_table(_name, _job_id):
    return Standalone.get_instance().dtable(_name, _job_id, partition=10)
...
class FederationRuntime(object):
    ...
    def remote(self, obj, name: str, tag: str, role=None, idx=-1):
        ...
        for _role, _partyIds in parties.items():
            for _partyId in _partyIds:
                ...
                if isinstance(obj, _RDDTable):
                    name = str(uuid.uuid1())
                    namespace = self.job_id
                    obj_dtable = obj.save_as(name, namespace, partition=obj._partitions, persistent=False)
                    _status_table.put(_tagged_key, (obj_dtable._type, name, namespace, obj_dtable._partitions))
                    
                elif isinstance(obj, _DTable):
    ...
    def get(self, name, tag, idx=-1):
        ...
        for party_id in party_ids:
            _tagged_key = self.__remote__object_key(self.job_id, name, tag, src_role, party_id, self.role,
                                                    self.party_id)
            _status_table.delete(_tagged_key)
        ...
        for r in results:
            if isinstance(r, tuple):
                ...
                rdd_table = Standalone.get_instance().table(name=r[1],
                                                            namespace=r[2], 
                ...
                rdd_table.count()
                rdd_table.destroy()
                rtn.append(rdd_table)                
            else:
                rtn.append(_object_table.get(r))
                _object_table.delete(r)
