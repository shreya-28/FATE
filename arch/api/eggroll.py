...
def init(job_id=None, mode: WorkMode = WorkMode.STANDALONE):
    ...
    elif mode == WorkMode.PYSPARKSTANDALONE:
        from arch.api.pyspark.standalone.eggroll import Standalone
        RuntimeInstance.EGGROLL = Standalone(job_id=job_id)    
    elif mode == WorkMode.PYSPARKCLUSTER:
        from arch.api.pyspark.cluster.eggroll import _EggRoll
        from arch.api.pyspark.cluster.eggroll import init as c_init
        c_init(job_id)
        RuntimeInstance.EGGROLL = _EggRoll.get_instance() 
...
def table(name, namespace, partition=1, persistent=True, create_if_missing=True, error_if_exist=False, use_serialize=True):
