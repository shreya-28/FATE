...
eeooff
	...
	ssh -tt $user@$bip << eeooff
cd $dir

sed -i "s#^fate.url=.*#fate.url=http://$tmip:9380#g" ./fateboard/conf/application.properties
sed -i "s#^spring.datasource.url=.*#spring.datasource.url=jdbc:mysql://$jdbcip:3306/$jdbcdbname?characterEncoding=utf8\&characterSetResults=utf8\&autoReconnect=true\&failOverReadOnly=false\&serverTimezone=GMT%2B8#g" ./fateboard/conf/application.properties
sed -i "s/^spring.datasource.username=.*/spring.datasource.username=$jdbcuser/g" ./fateboard/conf/application.properties
sed -i "s/^spring.datasource.password=.*/spring.datasource.password=$jdbcpasswd/g" ./fateboard/conf/application.properties

for IP in ${ips[@]}
do
	echo $IP
	sed -i "/$IP/d" ./fateboard/conf/ssh.properties
	sed -i "\$a\\$IP=app|Apps@123|360005" ./fateboard/conf/ssh.properties
done
